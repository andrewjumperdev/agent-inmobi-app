    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1.5">
        {pillars.map((p) => {
          ...
          const color = p !== "all" ? PILLAR_COLORS[p as ContentPillar] : "var(--muted-foreground)";
          return (
            <button
              key={p}
              onClick={() => onPillar(p)}
              className="flex items-center gap-1.5 rounded-lg border px-2.5 py-1 font-label text-[10px] uppercase tracking-widest transition-all"
              style={{
                borderColor: active ? `${color}50` : "var(--app-border)",
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            style={{
              borderColor: copied ? "color-mix(in oklab, var(--success) 40%, transparent)" : "var(--app-border)",
              backgroundColor: copied ? "color-mix(in oklab, var(--success) 8%, transparent)" : "transparent",
      ...
      <div className="flex items-start justify-between">
        ...
        {piezas.length > 0 && (
          ...
          >
            ...
            <p className="font-label text-[10px] uppercase tracking-widest" style={{ color: "var(--muted-foreground)" }}>
              piezas
            </p>
          </div>
        )}
